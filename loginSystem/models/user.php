...
class User {
    ...
    function display_favourite_posts() {
        ...
        // Retrieve post IDs from the favourites table for a specific user
    $sql = "SELECT post_id FROM favourites WHERE user_id = $user_id";
    $result = $this->connection->query($sql);
    $postIds = array();
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $postSql = "SELECT * FROM posts WHERE id IN (" . implode(",", $postIds) . ")";
        $postResult = $this->connection->query($postSql);

        while ($postRow = $postResult->fetch_assoc()) {
            echo '<div class="post">';
            echo '<div class="title">' . $postRow['title'] . '</div>';
            echo '<div class="content">' . $postRow['content'] . '</div>';
            echo '</div>';
        }
    } else {
